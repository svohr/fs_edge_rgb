...
rows = [
    ...
    ["hk1", "hk2", "gap_half", "tilde", "1", "2", "3", "4", "5", "6", "gap_double", "7", "8", "9", "0", "hyph", "=", "bspc", "home"],
    ...
    ["hk7", "hk8", "gap_half", "lshft", "z", "x", "c", "v", "b", "gap_double", "n", "m", "com", "per", "/", "rshft", "up", "pdn"],
    ["hk9", "hk10", "gap_half", "lctrl", "lwin", "lalt", "lspc", "gap_single", "rspc", "ralt", "rctrl", "lft", "dwn", "rght"],
]
...
board_width = sum(key_size.get(k, 1.0) for k in rows[0])
...
def scale_coords(c, dims):
    return (int((c[0] / board_width) * dims[0]), int((c[1] / board_height) * dims[1]))
...
im = Image.open("seabass.png")

for k in keys_all:
    coords = get_key_image_coords(k, im.size)
    color = im.getpixel(coords)
    print(f"[{k}]>[{color[0]}][{color[1]}][{color[2]}]")
